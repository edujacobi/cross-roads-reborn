...
import { StockList, STOCK_GLOBAL_SUPPLY, STOCK_MAX_SHARES, StockId, getStockList } from "../interfaces/Stocks";
...
export class Stock {
	...
	static async Initialize() {
		for (const stockDef of getStockList()) {
			const [_, created] = await StockMarket.findOrCreate({
				where: { companyId: stockDef.Id },
				defaults: {
					companyId: stockDef.Id,
			...
			if (created) {
				Log.Info(`Stock ${stockDef.CompanyName} (${stockDef.Id}) initialized with price ${stockDef.InitialPrice}.`);
			}
		}
	...
	static async RefreshMarket() {
		...
		for (const stock of stocks) {
			...
			stock.history = JSON.stringify(history);
	...
	async GetPortfolio() {
		...
		for (const s of stocks) {
			const marketData = await StockMarket.findByPk(s.companyId);
			portfolio.push({
	...
	async Buy(companyId: StockId, quantity: number) {
		const s = Strings[this.User.Language];
		const stockData = StockList[companyId];

		if (quantity <= 0) {
			Log.Warning(`User ${this.User.Nickname} (Id: ${this.User.Id}) tried to buy ${quantity} shares of ${stockData.CompanyName} with an invalid quantity.`);
			return {
				success: false,
		...
		if (totalShares + quantity > STOCK_MAX_SHARES) {
			Log.Warning(`User ${this.User.Nickname} (Id: ${this.User.Id}) tried to buy ${quantity} shares of ${stockData.CompanyName} but reached the global limit of ${STOCK_MAX_SHARES} shares. Total shares: ${totalShares}.`);
			return {
				success: false,
		...
		// Get stock data
		const stock = await StockMarket.findByPk(companyId);
		if (!stock) {
			Log.Warning(`User ${this.User.Nickname} (Id: ${this.User.Id}) tried to buy ${quantity} shares of ${stockData.CompanyName} but the stock was not found.`);
			return {
				success: false,
		...
		if (stock.availableShares < quantity) {
			Log.Warning(`User ${this.User.Nickname} (Id: ${this.User.Id}) tried to buy ${quantity} shares of ${stockData.CompanyName} but there were not enough shares available. Available: ${stock.availableShares}.`);
			return {
				success: false,
		...
		if (this.User.Money < cost) {
			Log.Warning(`User ${this.User.Nickname} (Id: ${this.User.Id}) tried to buy ${quantity} shares of ${stockData.CompanyName} but did not have enough money. Needed: ${cost}. Has: ${this.User.Money}.`);
			return {
				success: false,
		...
		const userStock = await UserStocks.findOne({
			where: { userId: this.User.Id, companyId: companyId },
		});
		...
		Log.Success(`User ${this.User.Nickname} (Id: ${this.User.Id}) bought ${quantity} shares of ${stockData.CompanyName} for ${formatMoney(cost, Language.English)}. Remaining money: ${formatMoney(this.User.Money, Language.English)}.`);
		return {
			success: true,
			message: s.buySuccess(quantity, stockData.CompanyName, formatMoney(cost, this.User.Language)),
		};
	}

	async Sell(companyId: StockId, quantity: number) {
		const s = Strings[this.User.Language];
		const stockData = StockList[companyId];

		if (quantity <= 0) {
			Log.Warning(`User ${this.User.Nickname} (Id: ${this.User.Id}) tried to sell ${quantity} shares of ${stockData.CompanyName} with an invalid quantity.`);
			return {
				success: false,
				message: s.invalidQuantity,
			};
		...
		if (!userStock || userStock.quantity < quantity) {
			Log.Warning(`User ${this.User.Nickname} (Id: ${this.User.Id}) tried to sell ${quantity} shares of ${stockData.CompanyName} but only owns ${userStock?.quantity || 0}.`);
			return {
				success: false,
		...
		const stock = await StockMarket.findByPk(companyId);
		if (!stock) {
			Log.Warning(`User ${this.User.Nickname} (Id: ${this.User.Id}) tried to sell ${quantity} shares of ${stockData.CompanyName} but the stock was not found.`);
			return {
				success: false,
				message: s.stockNotFound,
			};
		...
		Log.Success(`User ${this.User.Nickname} (Id: ${this.User.Id}) sold ${quantity} shares of ${stockData.CompanyName} for ${formatMoney(revenue, Language.English)}. Profit: ${formatMoney(profit, Language.English)}.`);
		return {
			success: true,
			message: s.sellSuccess(quantity, stockData.CompanyName, formatMoney(revenue, this.User.Language), formatMoney(profit, this.User.Language)),
		};
	}
}
...
const Strings = {
	[Language.English]: {
		...
		buySuccess: (qty: number, company: string, cost: string) => `You successfully bought ${qty} shares of **${company}** for ${cost}.`,
		sellSuccess: (qty: number, company: string, revenue: string, profit: string) => `You successfully sold ${qty} shares of **${company}** for ${revenue}. Profit/Loss: ${profit}.`,
	},
	[Language.Portuguese]: {
		...
		buySuccess: (qty: number, company: string, cost: string) => `Você comprou com sucesso ${qty} ações de **${company}** por ${cost}.`,
		sellSuccess: (qty: number, company: string, revenue: string, profit: string) => `Você vendeu com sucesso ${qty} ações de **${company}** por ${revenue}. Lucro/Prejuízo: ${profit}.`,
	},
	[Language.Spanish]: {
		...
		buySuccess: (qty: number, company: string, cost: string) => `Compraste con éxito ${qty} acciones de **${company}** por ${cost}.`,
		sellSuccess: (qty: number, company: string, revenue: string, profit: string) => `Vendiste con éxito ${qty} acciones de **${company}** por ${revenue}. Ganancia/Pérdida: ${profit}.`,
	},
};
