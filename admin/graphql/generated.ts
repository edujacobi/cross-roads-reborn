...
export type GetUserDetailQuery = { user: { id: string, nickname: string, money: number, avatarUrl: string | null, specialCoin: number, gangId: number | null, class: number, className: string, attack: number, defense: number, isVip: boolean, vipEternal: boolean, vipTime: string | null, language: string, isInHospital: boolean, hospitalTime: string | null, isInPrison: boolean, prisonTime: string | null, isWorking: boolean, jobEndsIn: string | null, isScavenging: boolean, isWanted: boolean, isInCasino: boolean, investmentId: number | null, situationId: number, dailyStreak: number, voteCount: number, items: Array<{ id: number, name: string, type: number, quantity: number, skin: number, remainingTime: string | null }> } | null };
