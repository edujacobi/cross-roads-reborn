...
<template>
  <div v-if="auth.loading.value" class="loading-screen">
    <div class="spinner" />
    <p>Carregando painel administrativo...</p>
  </div>

  <div v-else-if="auth.isAuthenticated.value" class="admin-layout">
    <!-- Sidebar -->
    <aside class="sidebar">
      <div class="brand">
      ...
      <div class="admin-profile">
        ...
        <div class="admin-details">
          ...
          <BaseBadge
            :variant="auth.isDeveloper.value ? 'developer' : 'moderator'"
          >
            {{ auth.user.value?.role }}
          </BaseBadge>
        </div>
        <button class="logout-btn" title="Encerrar sessão" @click="auth.logout()">
          <LogOut :size="18" />
        </button>
      </div>
    </aside>
    ...
    <div class="main-content-wrapper">
      <header class="top-header">
        ...
        <div class="header-role-indicator">
          <span v-if="auth.isDeveloper.value" class="role-hint dev">
            Permissão: Leitura & Escrita
          </span>
          <span v-else class="role-hint mod">
