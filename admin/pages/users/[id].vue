...
<template>
  <div class="user-detail-page">
    ...
    <!-- Alert Banner -->
    <div v-if="feedback" :class="['feedback-banner', feedback.type]">
      <CheckCircle2 v-if="feedback.type === 'success'" :size="18" />
    ...
    <div v-else class="user-content">
        ...
        <div class="profile-main-info">
          ...
          <div>
            <div class="name-row">
              ...
              <BaseBadge v-if="user.vipEternal" variant="vip">VIP Eterno</BaseBadge>
              <BaseBadge v-else-if="user.isVip" variant="vip">VIP</BaseBadge>
            </div>
      ...
      <BaseCard title="Ações Administrativas" class="actions-card">
        <template #actions>
          <span v-if="!auth.isDeveloper.value" class="read-only-badge">
      ...
      <div class="info-columns">
        <!-- Status Card -->
        <BaseCard title="Situação & Estados" class="status-summary-card">
          <div class="status-rows">
            <div class="status-row">
              <span class="row-label">Hospital</span>
              <BaseBadge v-if="user.isInHospital" variant="danger">
                Internado até {{ new Date(user.hospitalTime).toLocaleTimeString() }}
              </BaseBadge>
              <BaseBadge v-else variant="success">Livre</BaseBadge>
            </div>

            <div class="status-row">
              <span class="row-label">Prisão</span>
              <BaseBadge v-if="user.isInPrison" variant="neutral">
            ...
            <div class="status-row">
              ...
              <BaseBadge v-if="user.isWorking" variant="warning">Trabalhando</BaseBadge>
              <BaseBadge v-else variant="neutral">Sem trabalho ativo</BaseBadge>
            </div>

            <div class="status-row">
              <span class="row-label">Garimpo</span>
              <BaseBadge v-if="user.isScavenging" variant="warning">Garimpando</BaseBadge>
              <BaseBadge v-else variant="neutral">Inativo</BaseBadge>
            </div>

            <div class="status-row">
              <span class="row-label">Procurado pela Polícia</span>
              <BaseBadge v-if="user.isWanted" variant="danger">Procurado</BaseBadge>
              <BaseBadge v-else variant="success">Ficha Limpa</BaseBadge>
            </div>
          </div>
        </BaseCard>

        <!-- Economy Card -->
        <BaseCard title="Economia" class="economy-summary-card">
      ...
      <!-- Inventory Section -->
      <BaseCard :title="`Inventário (${user.items.length} Itens)`" class="inventory-card">
        <div v-if="user.items.length === 0" class="empty-inv">
          <Package :size="32" />
          <p>O jogador não possui itens no inventário.</p>
        </div>

        <div v-else class="items-grid">
          <div v-for="item in user.items" :key="item.id" class="item-card">
            ...
            <div class="item-stats">
              <span v-if="item.attack > 0" class="stat atk">ATK: +{{ item.attack }}</span>
              <span v-if="item.defense > 0" class="stat def">DEF: +{{ item.defense }}</span>
    ...
    >
        ...
        <div class="form-group">
          ...
          <div class="radio-group">
            <label class="radio-label">
              <input v-model="moneyMode" type="radio" value="ADD" />
              <span>Adicionar ao saldo atual</span>
            </label>
            <label class="radio-label">
              <input v-model="moneyMode" type="radio" value="SET" />
      ...
      <template #footer>
        <BaseButton variant="ghost" @click="isMoneyModalOpen = false">Cancelar</BaseButton>
        <BaseButton variant="primary" :disabled="moneyLoading" @click="handleSetMoney()">
          Confirmar Alteração
        </BaseButton>
      </template>
    </BaseModal>
    ...
    >
      <div class="modal-form">
        <div class="form-group">
          <label>Escolha o Cooldown</label>
          <select v-model="selectedCooldown" class="form-select">
      ...
      <template #footer>
        <BaseButton variant="ghost" @click="isCooldownModalOpen = false">Cancelar</BaseButton>
        <BaseButton variant="primary" :disabled="cdLoading" @click="handleResetCooldown()">
          Resetar Cooldown
        </BaseButton>
      </template>
    </BaseModal>
    ...
    >
      ...
      <template #footer>
        <BaseButton variant="ghost" @click="isActionModalOpen = false">Cancelar</BaseButton>
        <BaseButton variant="primary" :disabled="actionLoading" @click="handleRemoveAction()">
          Remover da Ação
        </BaseButton>
      </template>
    </BaseModal>
    ...
    .item-card {
      ...
      .item-stats {
        ...
        .stat {
          ...
          &.atk { color: $color-danger; }
          &.def { color: $color-info; }
          &.price { color: $color-success; }
        }
      }
    }
  }
