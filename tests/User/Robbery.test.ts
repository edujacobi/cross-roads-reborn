...
import { LocationRobberyStrategy } from "#core/models/strategies/robbery/LocationRobberyStrategy";
...
describe("Robbery", () => {
	...
	describe("When testing if user can rob another user", () => {
		...
		it("Should fail if user tries to rob themselves", async () => {
			const robbery = new UserRobberyStrategy(user, user);
			const result = await robbery.CanRob();
		...
		it("Should fail if attacker and defender are in same gang", async () => {
			user.GangId = 1;
			defender.GangId = 1;
			const robbery = new UserRobberyStrategy(user, defender);
			const result = await robbery.CanRob();
			expect(result.canRob).toBe(false);
	...
	describe("When testing if user can rob a location", () => {
		...
		it("Should fail if low ATK", async () => {
			const robberyLoc = new LocationRobberyStrategy(user, LocationId.SmallBank);
			const result = await robberyLoc.CanRob();
			expect(result.canRob).toBe(false);
